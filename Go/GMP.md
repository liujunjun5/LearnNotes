...
## 什么是Go scheduler
是一个调度算法，内部定义了GMP的调度流程，如何查找G，如何切换g0、g
## 调度策略是怎么样的
所谓调度指的是多个G之间如何切换问题。  
抢占式调度，sysmon会监控运行时间过长的（10ms）g，添加上标记位：在go1.14之前g会在每一次执行函数之前检查是否有该标记，如果有释放g换为g0调度其他g，但是存在问题————空循环这类不调用函数的无法释放。  
1.14之后会向M发送信号，有专门的g处理这个信号，将g切换
## 调度的时机是什么时候
- 运行时间过长
- 主动释放
- 阻塞
...
不能。P给定了一个无锁的本地队列，性能
## M和P分别在什么时候创建
- P在m0创建，初始化的时候MaxGoProc
- M
  - 不适应P的时候
  - 没有空闲的M
  - 默认上线10000
## m0的作用
创建垃圾回收器、调度器、main函数，运行期间和普通的m一样。最后还会负责资源的回收，清理。
## g0是什么，为什么不能去掉
和m绑定的g，负责g的调度，有专门的栈（8KB）。
不能去掉——————互相递归调用、空间不足  
## g0栈和用户栈是如何进行切换的？
mcall切换到g0，gogo 切换到g
本质上是pc和sp的切换
